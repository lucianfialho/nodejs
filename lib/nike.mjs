  ...
  async scrape(page) {
    const promise = new Promise(async (resolve) => {
      page.once("domcontentloaded", async () => {
        const sneaker = await page.evaluate(() => {
          ...
          // Extração detalhada do preço
          const priceInfo = productData.priceInfos || {};
          const promotionalPrice = priceInfo.promotionalPrice || null;
          const originalPrice = priceInfo.price || null;
          const priceFormatted =
            priceInfo.promotionalPriceFormatted ||
            priceInfo.priceFormatted ||
            null;
          const discount = priceInfo.discount || null;
          const pixDiscount = priceInfo.pixDiscount || {};

          // Preparando os dados do sneaker
          return {
            ...
            discount: discount,
            pixPrice: pixDiscount.price || null,
            pixPriceFormatted: pixDiscount.priceFormatted || null,
            totalDiscountPercentage:
              pixDiscount.totalDiscountPercentage || discount || null,
            ...
            availableSizes: productData.sizes
              .filter((s) => s.hasStock)
              .map((s) => s.description),
            reviewsCount: productData.reviewsSummary?.count || 0,
            reviewsRating: productData.reviewsSummary?.rating || 0,
            colorVariations: productData.colorVariations.map((variation) => ({
